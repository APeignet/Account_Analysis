...
Original file is located at
    https://colab.research.google.com/drive/18m91VI2GisAlX8giIuwn_bPE4Sw1R40M
...
analysisPopSankeyS1 = 'GB - IP - COTY - Hugo Boss - REACH'
analysisProductS1 = 'Boss Bottle Eau de Parfum'
...
analysisPopSankeyS2 = 'GB - IP - COTY - Hugo Boss - REACH'
analysisProductS2 = 'Boss Bottle Eau de Parfum'
...
if analysisByPopLCR3 not in pop(analysisAccount, analysisOperator,analysisCountry, 'N', 'N','all','null','null','N')['PoP'].tolist() :   
  analysisByPopLCR3 = pop(analysisAccount, analysisOperator,analysisCountry, 'N', 'N','all','null','null','N')['PoP'].tolist()[0]

if analysisPopSankeyS1 not in pop(analysisAccount, analysisOperator,analysisCountry, 'N', 'N','all','null','null','N')['PoP'].tolist() :   
  analysisPopSankeyS1 = pop(analysisAccount, analysisOperator,analysisCountry, 'N', 'N','all','null','null','N')['PoP'].tolist()[0]

if analysisPopSankeyS2 not in pop(analysisAccount, analysisOperator,analysisCountry, 'N', 'N','all','null','null','N')['PoP'].tolist() :   
...
def analysisProductMinItemQty(byPop, byOperator, byCountry) : 
  ...
  df1 = necessaryData_account1OnlyAccount.assign()
  ...
  df3 = df.join(df2.set_index('pid'), on='pid')
  df8 = df3.assign()
  df8 = df8[['product_label','prc']].drop_duplicates().groupby(['product_label'], as_index=False).mean()
  df4 = df3.assign()
  df4 = df4.groupby(['product_label'], as_index=False).size().sort_values(['size'], ascending=False)
  df4 = df4.rename(columns={'size' : 'leads'})
  df5 = df3.assign()
  df5 = df5.groupby(['product_label'], as_index=False).sum()
  df5 = df5.join(df4.set_index('product_label'), on='product_label')
  df5 = df5[['product_label', 'minimum_item_qty','leads']]
  df5 = df5.join(df8.set_index('product_label'), on='product_label')
  ...
  if (byCountry == 'null') & (byOperator == 'null') & (byPop == 'null') : 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads (threshold = {} minimum item quantity)'.format(threshold))
  elif (byCountry != 'null') & (byOperator == 'null') & (byPop == 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads in {} (threshold = {} minimum item quantity)'.format(byCountry,threshold))
  elif (byCountry != 'null') & (byOperator != 'null') & (byPop == 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the operator {} in {} (threshold = {} minimum item quantity)'.format(byOperator,byCountry,threshold))
  elif (byCountry != 'null') & (byOperator == 'null') & (byPop != 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the PoP {} in {} (threshold = {} minimum item quantity)'.format(byPop,byCountry,threshold))
  elif (byCountry == 'null') & (byOperator != 'null') & (byPop != 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the operator {} for the PoP {} (threshold = {} minimum item quantity)'.format(byOperator,byPop,threshold))
  elif (byCountry == 'null') & (byOperator != 'null') & (byPop == 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the operator {} (threshold = {} minimum item quantity)'.format(byOperator,threshold))
  elif (byCountry == 'null') & (byOperator == 'null') & (byPop != 'null') :
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the PoP {} (threshold = {} minimum item quantity)'.format(byPop,threshold))
  elif (byCountry != 'null') & (byOperator != 'null') & (byPop != 'null'): 
    fig = px.scatter(df5, x="leads", y="minimum_item_qty", log_x=True,
                 hover_name="product_label", hover_data=["minimum_item_qty"],color="product_label", size="prc",size_max=25,title= 'Repartition of products per minimum item quantity per leads for the operator {} in {} for the PoP {} (threshold = {} minimum item quantity)'.format(byOperator,byCountry,byPop,threshold))
...
# Indications : 
# Copy of the above function. It is only used to display the customer’s product list to correct product filters (if incorrect)

def analysisProductMinItemQtyForName(byPop, byOperator, byCountry) : 
...
analysisProductMinItemQtyForName(analysisByPopP1, analysisByOperatorP1, analysisByCountryP1)

# If the filters are filled incorrectly at the top, for each graph that needs a product name, the 1st product of the analysisProductMinItemQtyForName function will be chosen by default

if analysisProductS1 not in analysisProductMinItemQtyForName(analysisByPopP1, analysisByOperatorP1, analysisByCountryP1)['product_label'].tolist() :   
  analysisProductS1 = analysisProductMinItemQtyForName(analysisByPopP1, analysisByOperatorP1, analysisByCountryP1)['product_label'].tolist()[0]

if analysisProductS2 not in analysisProductMinItemQtyForName(analysisByPopP1, analysisByOperatorP1, analysisByCountryP1)['product_label'].tolist() :   
  analysisProductS2 = analysisProductMinItemQtyForName(analysisByPopP1, analysisByOperatorP1, analysisByCountryP1)['product_label'].tolist()[0]
